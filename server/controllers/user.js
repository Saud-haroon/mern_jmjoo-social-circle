...
export const signin = async (req, res) => {
    ...
    try {
        ...
        if (!exisitingUser) return res.status(404).json({ message: "User dosen't exists." });
...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (password !== confirmPassword) return res.status(400).json({ message: "Password don't Matchs" });
